...
[ -z "$BOARD" ] && (echo "ERROR: You have to choose one board"; exit)
...
if [ "$KERNEL_ONLY" != "yes" ]; then
	...
	# exit the script on cancel
	[ -z "$RELEASE" ] && (echo "ERROR: You have to choose one distribution"; exit)
	...
	# exit the script on cancel
	[ -z "$BUILD_DESKTOP" ] && (echo "ERROR: You need to choose"; exit)
...
# exit the script on cancel
[ -z "$BRANCH" ] && (echo "ERROR: You have to choose one branch"; exit)

# don't compile external modules on mainline
[ "$BRANCH" == "next" ] && EXTERNAL="no"

# back to normal
unset IFS

# default console if not set
[ "$CONSOLE_CHAR" == "" ] && CONSOLE_CHAR="UTF-8"
...
[ -n "$MISC1" ] && fetch_from_github "$MISC1" "$MISC1_DIR"
[ -n "$MISC2" ] && fetch_from_github "$MISC2" "$MISC2_DIR"
[ -n "$MISC3" ] && fetch_from_github "$MISC3" "$MISC3_DIR"
[ -n "$MISC4" ] && fetch_from_github "$MISC4" "$MISC4_DIR" "master"
[ -n "$MISC5" ] && fetch_from_github "$MISC5" "$MISC5_DIR"
